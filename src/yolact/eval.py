...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        idx = t[1].argsort(0, descending=True)[:1] #args.top_k
    ...
    num_dets_to_consider = min(1, classes.shape[0])
    for j in range(num_dets_to_consider):
        if scores[j] < 0.2: # args.threshold
...
def prep_benchmark(dets_out, h, w):
    ...
    with timer.env('Copy'):
        classes, scores, boxes, masks = [x[:1] for x in t]
